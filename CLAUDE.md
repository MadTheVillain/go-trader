...
- Per-strategy DD: spot/options/futures use peak-relative; perps uses `unrealized_loss / deployed_margin` when positions open. `RecordTradeResult` only updates `DailyPnL` and `ConsecutiveLosses` — lifetime trade counts come from SQLite, not `RiskState` (#472).
