...
| Trailing stop (%) | `trailing_stop_pct` | Hyperliquid perps only — distance from high-water mark; mutually exclusive with the other HL stop/trailing owners (`stop_loss_pct`, `stop_loss_margin_pct`, `trailing_stop_atr_mult`) when positive (#501/#502/#507). Applies in both live and paper (#532). Capped at 50%. Explicit `0` disables. |
